...
if __name__ == "__main__":
    ...
    try:
        ...
        file_name = args.place
        directory = args.directory

        city_name = file_name.replace('poi_tw_', '')

        city = ox.gdf_from_place(city_name)
        df = pd.read_pickle(directory + "%s.pkl" % file_name)
        df2 = pd.read_pickle(
            directory + ("{0}/independence_{0}.pkl".format(file_name)))
        ...
        df3['geohash'] = df3.apply(lambda x: gh.encode(
            x['location'][1], x['location'][0], precision=6), axis=1)
        ...
        df_heat.total.fillna(0, inplace=True)
        ...
        fig.savefig(directory + "{0}/{1}_indep_grid.png".format(file_name, city_name))
